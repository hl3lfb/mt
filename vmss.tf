resource "azurerm_virtual_machine_scale_set" "user20vmss" {
name = "user20vmss"
...
ssh_keys {
    path = "/home/hl3lfb/.ssh/authorized_keys"   ## pwd 실행후 경로설정 ex) /home/user20 등 
...
network_profile {
        ...
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.user20-bpepool.id]
        load_balancer_inbound_nat_rules_ids = [azurerm_lb_nat_pool.lbnatpool.id]
    }
        network_security_group_id = azurerm_network_security_group.user20nsg.id
}
tags = {
    environment = "staging"
    }
